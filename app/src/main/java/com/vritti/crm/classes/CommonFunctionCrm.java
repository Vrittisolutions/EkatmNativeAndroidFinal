    ...
    public List<BusinessSegmentbean> getBusinessSegmentbean() {
        ...
        if (cur.getCount() > 0) {
            ...
            do {
                ...
                bean.setSegmentCode(cur.getString(cur.getColumnIndex("SegmentCode")));
                bean.setSegmentDescription(cur.getString(cur.getColumnIndex("SegmentDescription")));
                bean.setFKParentBusiSegmentID(cur.getString(cur.getColumnIndex("FKParentBusiSegmentID")));
...
*/
    ...
    public int getprospectcitycnt() {
        String countQuery = "SELECT  * FROM "
                + db.TABLE_CITY_PROSPECT;
        int count = 0;
        SQLiteDatabase sql = db.getReadableDatabase();
        Cursor cursor = sql.rawQuery(countQuery, null);
        if (cursor != null && !cursor.isClosed()) {
    ...
    public int getprospectterritorycnt(){
        String countQuery = "SELECT  * FROM "
                + db.TABLE_Territory;
        int count = 0;
        SQLiteDatabase sql = db.getReadableDatabase();
    ...
    public int getSalesFamilyProuctcount() {
        String countQuery = "SELECT  * FROM "
                + db.TABLE_SALES_FAMILY_PRODUCT;
        int count = 0;
        SQLiteDatabase sql = db.getReadableDatabase();
        Cursor cursor = sql.rawQuery(countQuery, null);
        if (cursor != null && !cursor.isClosed()) {
    ...
    public int getProuctdetailcount(String ItemDesc) {
        String countQuery = "SELECT  * FROM "
                + db.TABLE_Product_Details_New + " WHERE ItemDesc='" + ItemDesc + "'";
        int count = 0;
        SQLiteDatabase sql = db.getReadableDatabase();
        Cursor cursor = sql.rawQuery(countQuery, null);
        if (cursor != null && !cursor.isClosed()) {
    ...
    public long addofflinedata(String url, String parameter,
                               int methodtype, String remark, String op,String Path,String FileName) {
        SQLiteDatabase sql = db.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put("linkurl", url);
        values.put("parameter", parameter);
        ...
        values.put("isUploaded", WebUrlClass.FlagisUploadedFalse);
        values.put("AttemptCount", 0);
        values.put("AttachmentPath", Path);
        values.put("AttachmentFileName", FileName);
        long a = sql.insert(db.TABLE_DATA_OFFLINE, null, values);
        String cnt = a + "";
        return a;
    }
    ...
    public List<SalesFamily> getSalesFamilyBean() {
        List<SalesFamily> list = new ArrayList<SalesFamily>();
        list.clear();
        //  Firmbean[] list = null;
        SQLiteDatabase sql = db.getReadableDatabase();
        String qry = "";
        qry += "SELECT * FROM " + db.TABLE_SALES_FAMILY_PRODUCT;
        ...
        if (cur.getCount() > 0) {
            ...
            do {
                SalesFamily bean = new SalesFamily();
                bean.setFamilyId(cur.getString(cur.getColumnIndex("FamilyId")));
                bean.setFamilyDesc(cur.getString(cur.getColumnIndex("FamilyDesc")));
                //bean.setId(cur.getString(cur.getColumnIndex("Id")));
                list.add(bean);
    ...
    public int getEntitycount_Prospect() {
        String countQuery = "SELECT  * FROM "
                + db.TABLE_Entity;
