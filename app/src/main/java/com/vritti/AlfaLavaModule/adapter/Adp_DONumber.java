...
public class Adp_DONumber extends RecyclerView.Adapter<Adp_DONumber.ViewHolder> {
    private List<PicklistNO> mList;
    private static MyClickListener myClickListener;
    static Context context;


    public Adp_DONumber(List<PicklistNO> list) {
        this.mList = list;
    }

    @Override
    public Adp_DONumber.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
    ...
    @Override
    public void onBindViewHolder(Adp_DONumber.ViewHolder holder, int i) {
        holder.tv_Grn_no.setText(mList.get(i).getPicklistNo());
        String remark=mList.get(i).getRemark();
        if (remark.equals("")){
            holder.tv_mo_no.setVisibility(View.GONE);
        }else {
            holder.tv_mo_no.setVisibility(View.VISIBLE);
            holder.tv_mo_no.setText("Remark : " + remark);
    ...
    public class ViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public void onClick(View v) {
            ...
            ((DOListActivity) context).setdonumber(mList.get(getPosition()).getPicklistNo(),
                    mList.get(getAdapterPosition()).getPick_listHdrId(),mList.get(getAdapterPosition()).getQCStatus());
