...
public class ApplyLeaveMainActivity extends AppCompatActivity {
    ...
    private void Setlistner() {
        btn_apply_leave_save.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (LeaveType.equalsIgnoreCase("OnDuty")) {
                    String fromDate = btn_from_date.getText().toString();
                    String toDate = btn_to_date.getText().toString();

                    String fromdate = formateDateFromstring("dd/MM/yyyy", "yyyy-MM-dd", fromDate);
                    String todate = formateDateFromstring("dd/MM/yyyy", "yyyy-MM-dd", toDate);

                    new ValidateDates().execute(fromdate, todate);
                    ...
                    if (compareDate_Diff()) {
                        ...
                        Toast.makeText(context, "You cannot apply leave more than available balance", Toast.LENGTH_SHORT).show();
                    }
                    //Validate();
                }
    ...
    private boolean compareDate_Diff() {
        ...
        long toDateMillis = date.getTime();
        long fromDateMillis = date1.getTime();

        diffInDays = (int) ((fromDateMillis - toDateMillis) / (1000 * 60 * 60 * 24));
        Log.i("Day cnt", String.valueOf(diffInDays));
        int tempbalance = (int) Float.parseFloat(balance);
        diffInDays++;

        if(diffInDays > tempbalance){
            return false;
        }
    ...
    class GetDochdrId extends AsyncTask<Integer, Void, Integer> {
        ...
        protected Integer doInBackground(Integer... params) {
            ...
            url =  CompanyURL + WebUrlClass.api_leave_dochdrId +"?Source=" + Source;

            // url = http://b207.ekatm.com/api/MyClaimAPI/getApproverList?DocMthdId=69
            try {
                res = ut.OpenConnection(url, ApplyLeaveMainActivity.this);
                res = res.replaceAll("\\\\", "");
                res = res.substring(1, res.length() - 1);
                ...
                try {
                    ...
                    if(ut.isNet(context)) {
                        ...
                        new StartSession(ApplyLeaveMainActivity.this, new CallbackInterface() {
                            ...
                            @Override
                            public void callfailMethod(String msg) {
                                ut.displayToast(ApplyLeaveMainActivity.this, msg);
                                //hidprogress();
                            }
                        });
                    }
    ...
    class DownloadmobNO extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String integer) {
            super.onPostExecute(integer);
            if (!res.equalsIgnoreCase("")) {
                Mobile.setText(columnValue.trim());
    ...
    private void getLeaveObj() {
        ...
        try {
            ...
            String que = "SELECT UserMasterId FROM " + db.TABLE_LEAVE_REPORTING_TO + " WHERE UserName='" + sp_leave_reportingto.getSelectedItem().toString() + "'";
            Cursor cur = sql.rawQuery(que, null);
            if (cur.getCount() > 0) {
                cur.moveToFirst();
                LeaveObj.put("ReportingTo", cur.getString(cur.getColumnIndex("UserMasterId")));
            }
    ...
    class ValidateDates extends AsyncTask<String,Void,String>{
        String res="";
        String url=null;
        String fromdate="",todate="";
        ...
        protected String doInBackground(String... params) {
          ...
          /*  fromdate = params[0];
            ...
            url = CompanyURL + WebUrlClass.api_LeaveDate_Validate +"?fromDt="+ URLEncoder.encode(params[0]) +"&toDate="+ URLEncoder.encode(params[0]);
        ...
        protected void onPostExecute(String s) {
            ...
            if(res.equalsIgnoreCase("")){
                ...
                Toast.makeText(ApplyLeaveMainActivity.this,"You cannot Apply Leave for Selected Date",Toast.LENGTH_SHORT).show();
            }
        }
    }
