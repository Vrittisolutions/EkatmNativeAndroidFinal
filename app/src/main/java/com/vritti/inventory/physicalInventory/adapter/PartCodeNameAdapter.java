...
public class PartCodeNameAdapter extends BaseAdapter implements Filterable {
    ...
    public PartCodeNameAdapter(Context parent,
    ...
    public View getView(final int position, View convertView,
        ...
        holder.textname.setText(list.get(position).getPartCode());
    ...
    public void filter(String charText) {
        ...
        if (charText.length() == 0) {
            ...
            for (PartCodeName wp : arraylist) {
                if (wp.getPartCode().toLowerCase(Locale.getDefault())
                        .contains(charText)) {
                    list.add(wp);
                }
            }
