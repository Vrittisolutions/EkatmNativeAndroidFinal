...
public class ContractActivity extends AppCompatActivity {
    ...
    AutoCompleteTextView txt_contractno;
    ...
    String contractHDRId = "", contractCode = "";

    ArrayList<PriceListBean> listItems;
    ArrayList<String> listContract;
    ...
    public void init(){
        ...
        text_cntr_quot.setHint("Contract No.");
        txt_contractno = findViewById(R.id.txt_contractno);
        ...
        listcontract = findViewById(R.id.listcontract);

        ut = new Utility();
        tcf = new Tbuds_commonFunctions(ContractActivity.this);
        cf = new CommonFunction(ContractActivity.this);
        ...
        listContract = new ArrayList<String>();
    ...
    public void setListeners(){
        ...
        btnadd.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                selected_listItems = ((SaleCommonAdapter) listcontract.getAdapter()).getAllCheckedItems();
                if(validate()){
                    if(selected_listItems.size() > 0){
                        ...
                        Intent intent = new Intent(ContractActivity.this, SalesItemActivity.class);
        ...
        txt_contractno.setOnTouchListener(new View.OnTouchListener() {
            @Override
            public boolean onTouch(View v, MotionEvent event) {
                txt_contractno.showDropDown();
                return false;
            }
        });

        txt_contractno.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                contractHDRId = ""; contractCode = "";
                try{
                    contractHDRId = listItems.get(position).getContractHdrId();
                    contractCode = listItems.get(position).getContractCode();

                    //call API pass pListHDRId to it and display it
                    if (isnet()) {
                        new StartSession(ContractActivity.this, new CallbackInterface() {
                            @Override
                            public void callMethod() {
        ...
        txt_contractno.addTextChangedListener(new TextWatcher() {
            @Override
                ...
                if(s.toString().length() >= 3){
                    ...
                    if (isnet()) {
                        new StartSession(parent, new CallbackInterface() {
                            @Override
                            public void callMethod() {
                                new DwnldContractListJSON().execute(text);
                            }
                            @Override
                            public void callfailMethod(String msg) {
    ...
    class DwnldContractListJSON extends AsyncTask<String, Void, String> {
        Object res;
        String response;
        ...
        @Override
        protected String doInBackground(String... params) {
            String url = CompanyURL + WebUrlClass.api_getContractNo+"?SearchText="+params[0];
            try {
                res = ut.OpenConnection(url);
                response = res.toString().replaceAll("\\\\", "");
                response = response.substring(1, response.length() - 1);
            ...
            if (response != "" || response != null || response != "error") {
                ...
                try{
                    ...
                    for (int i = 0; i < pdata.length; i++) {
                        ...
                        String hdrId = data[1].split("\"")[0].toString();

                        PriceListBean pBean = new PriceListBean();
                        pBean.setContractHdrId(hdrId);
                        pBean.setContractCode(code);
                        listContract.add(code);
                        listItems.add(pBean);
                       //tcf.insertPricelist(hdrId,code,desc);
                    }

                    ArrayAdapter adapter = new ArrayAdapter(parent,android.R.layout.simple_spinner_item,listContract);
                    txt_contractno.setAdapter(adapter);
                    txt_contractno.showDropDown();
    ...
    class DownloadItemsListJSON extends AsyncTask<String, Void, String> {
        ...
        @Override
        protected String doInBackground(String... params) {
            String url = CompanyURL + WebUrlClass.api_getContractItems+"?SOContractHeaderId="+contractHDRId;
            try {
                res = ut.OpenConnection(url);
                response = res.toString().replaceAll("\\\\", "");
                response = response.substring(1, response.length() - 1);
        ...
        protected void onPostExecute(String integer) {
            ...
            if (response != "" || response != null || response != "error") {
                ...
                Toast.makeText(parent,"No Contracts available",Toast.LENGTH_SHORT).show();
            }
        }
    }
    ...
    public void displayItemsList(String response){
        ...
        try{
            ...
            for (int i = 0; i < jResults.length(); i++) {
                ...
                PriceListBean pbean = new PriceListBean();
                pbean.setSOContractHDRId(jObj.getString("SOContractHeaderId"));
                pbean.setSOContractDtlId(jObj.getString("SOContractDetailid"));
                ...
                pbean.setUOMMasterId(jObj.getString("UOMMasterId1"));
            ...
            cAdapter = new SaleCommonAdapter(this,listItems,"Contract",taxClassArrayList,taxClsStringList);
            listcontract.setAdapter(cAdapter);
    ...
    public JSONArray createJSON(int size){
        ...
        for(int i=0; i<size; i++){
           ...
           /* DiscAmount = String.valueOf(Float.parseFloat(edtlineamt.getText().toString()) -
            ...
            ContractHdrId = selected_listItems.get(i).getContractHdrId();
    ...
    public boolean validate() {
        ...
        if(txt_contractno.getText().toString().equalsIgnoreCase("")){
            Toast.makeText(ContractActivity.this, "Please fill all details", Toast.LENGTH_SHORT).show();
            val = false;
            return val;
        }else if(txt_contractno.getText().toString().equalsIgnoreCase("") ||
                txt_contractno.getText().toString().equalsIgnoreCase(null)){
            Toast.makeText(ContractActivity.this, "Select contract no.", Toast.LENGTH_SHORT).show();
            val = false;
            return val;
        }else {
            for(int i =0; i<selected_listItems.size();i++){
                ...
                if(taxCls.equalsIgnoreCase("") || taxCls.equalsIgnoreCase(null)){
                    val = false;
                    Toast.makeText(ContractActivity.this, "Select tax class for selected item", Toast.LENGTH_SHORT).show();
                }else {
                    val = true;
