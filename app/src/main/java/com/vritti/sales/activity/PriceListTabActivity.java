...
public class PriceListTabActivity extends AppCompatActivity {
    private Context parent;
    Toolbar toolbar;
    ProgressBar progressBar;
    EditText txt_startdate,txt_del_date,txt_doordate,txt_pcode,txt_pdesc,edt_disc,txt_taxclass;
    ...
    ArrayList<PriceListBean> list_cntract;
    SaleItemListAdapter saleAdapter;
    ...
    public void init(){
        parent = PriceListTabActivity.this;
        ...
        ut = new Utility();
        tcf = new Tbuds_commonFunctions(PriceListTabActivity.this);
        cf = new CommonFunction(PriceListTabActivity.this);
    ...
    public void setListeners(){
        swtincltax.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                if(selTaxDesc == ""){
                    Toast.makeText(PriceListTabActivity.this,"Select tax class first",Toast.LENGTH_LONG).show();
                }else {
                    ...
                    //selTaxDesc = "SGST 9% + CGST 9% OUTPUT";    //get selected taxdesc here
                    txt_taxclass.setText(selTaxDesc);

                    selTaxId = getTaxFromDatabase2(selTaxDesc);
                    ...
                    if(isInclusiveTax==true){
                        for(int i=0; i<listItems.size();i++){
                            float urate = Float.parseFloat(listItems.get(i).getBaseRate());
                            float newurate = InclusiveExclusiveTaxCalc.calcRateInclTax(isInclusiveTax,urate,selTaxDesc,0);
                            listItems.get(i).setBaseRate(String.format("%.2f",newurate));
                            listItems.get(i).setMRP(String.format("%.2f",urate));
                        }

                        cAdapter = new SaleCommonAdapter(PriceListTabActivity.this,listItems,"PriceList",taxClassArrayList,taxClsStringList);
                        list_items.setAdapter(cAdapter);
                    ...
                    isInclusiveTax = false;
                        // setTaxClsToAllItems(TaxClassMasterId,selTaxDesc,selTaxCode);
                         for(int i=0; i<listItems.size();i++){
                             float urate = Float.parseFloat(listItems.get(i).getMRP());
                             listItems.get(i).setBaseRate(String.format("%.2f",urate));
                             listItems.get(i).setMRP(String.format("%.2f",urate));
                         }
        ...
        btnadd.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(validate()){
                    selected_listItems = ((SaleCommonAdapter) list_items.getAdapter()).getAllCheckedItems();
                    if(selected_listItems.size() > 0){
                        ...
                        Intent intent = new Intent(PriceListTabActivity.this, SalesItemActivity.class);
        ...
        txt_taxclass.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                intent.putExtra("callFrom","PriceList");
                startActivityForResult(intent,TAXDATA);
            }
        });

        txt_pcode.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(parent,PriceListActivity.class);
                intent.putExtra("key","PriceListCode");
                intent.putExtra("CallFrom","PriceListTab");
    ...
    public void displayItemsList(String response){
        ...
        try{
            ...
            for (int i = 0; i < jResults.length(); i++) {
                ...
                pbean.setPriceListHdrID(jObj.getString("PriceListHdrID"));
                pbean.setPriceListDtlID(jObj.getString("PriceListDtlID"));
                ...
                pbean.setQtyLine("0");
                pbean.setBaseRate(jObj.getString("BaseRate"));
                pbean.setLineAmt("0");
                pbean.setDiscountPer(jObj.getString("DiscountPer"));
            ...
            cAdapter = new SaleCommonAdapter(this,listItems,"PriceList",taxClassArrayList,taxClsStringList);
            list_items.setAdapter(cAdapter);
    ...
    public String getTaxFromDatabase2(String taxClsDesc){
    ...
    class DownloadItemsListJSON extends AsyncTask<String, Void, String> {
        ...
        @Override
        protected String doInBackground(String... params) {
            String url = CompanyURL + WebUrlClass.api_getPricelistItems+"?PriceListHdrID="+pListHDRId;
            try {
                res = ut.OpenConnection(url);
                response = res.toString().replaceAll("\\\\", "");
                response = response.substring(1, response.length() - 1);
    ...
    public boolean validate() {
        ...
        if(txt_pcode.getText().toString().equalsIgnoreCase("") &&
                txt_pdesc.getText().toString().equalsIgnoreCase("") &&
                txt_taxclass.getText().toString().equalsIgnoreCase("")){
            Toast.makeText(PriceListTabActivity.this, "Please fill all details", Toast.LENGTH_SHORT).show();
            val = false;
            return val;
        }else if(txt_pcode.getText().toString().equalsIgnoreCase("") ||
                txt_pcode.getText().toString().equalsIgnoreCase(null)){
            Toast.makeText(PriceListTabActivity.this, "Select price list code", Toast.LENGTH_SHORT).show();
            val = false;
            return val;
        }else if(txt_pdesc.getText().toString().equalsIgnoreCase("") ||
                txt_pdesc.getText().toString().equalsIgnoreCase(null)){
            Toast.makeText(PriceListTabActivity.this, "Select price list description", Toast.LENGTH_SHORT).show();
            val = false;
            return val;
        }else if(txt_taxclass.getText().toString().equalsIgnoreCase("") ||
                txt_taxclass.getText().toString().equalsIgnoreCase(null)){
            Toast.makeText(PriceListTabActivity.this, "Select tax class", Toast.LENGTH_SHORT).show();
            val = false;
            return val;
        }else {
            val = true;
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == TAXDATA && resultCode == TAXDATA) {
            ...
            txt_pcode.setText(pListCode);

            //call API pass pListHDRId to it and display it
            if (isnet()) {
                new StartSession(PriceListTabActivity.this, new CallbackInterface() {
                    @Override
                    public void callMethod() {
