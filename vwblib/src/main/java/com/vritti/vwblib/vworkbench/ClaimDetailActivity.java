           ...
           /* progressDialog = new ProgressDialog(ActivityLogIn.this);
            ...
            progressDialog.show();*/
                ...
                private void getclaimDtlsobjTotalobj() {
                    ...
                    int total = (int) (Integer.parseInt(ed_local.getText().toString()) + Integer.parseInt(ed_food.getText().toString())
                                                + Integer.parseInt(ed_stay.getText().toString()) + Integer.parseInt(ed_phone.getText().toString())
                                                + Integer.parseInt(ed_Maintenance.getText().toString()) + Double.parseDouble(ed_travel.getText().toString()));

                    bean.setAmount(total + "");
                    if (getIntent().hasExtra("Action")) {
                        if (getIntent().getStringExtra("Action").equalsIgnoreCase("Edit")) {
                            lsCalimDetails.remove(getIntent().getIntExtra("Position", 0));
                            lsCalimDetails.add(getIntent().getIntExtra("Position", 0), bean);
                        }
                    } else {
