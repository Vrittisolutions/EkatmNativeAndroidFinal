...
public class MyTeamMemberTimesheetActivity extends AppCompatActivity {
    ...
    private int getAssgncnt() {
        SQLiteDatabase sql = db.getWritableDatabase();
        String query = "SELECT ActivityName, Assigned_By,TotalHoursBooked, FormatStDt, FormatEndDt, ProjectName, PriorityIndex, HoursRequired ,StartDate,EndDate,FormatStDt,FormatEndDt,SourceType,Status FROM '" + db.TABLE_ACTIVITYMASTER_TEAM + "' WHERE UserMasterId ='" + UserMasterId + "'";
        Cursor cur = sql.rawQuery(query, null);
        if (cur.getCount() > 0) {
            return cur.getCount();
        }
    ...
    private int getcriticalcnt() {
        SQLiteDatabase sql = db.getWritableDatabase();

        String query = "SELECT ActivityName,Assigned_By, TotalHoursBooked, FormatStDt, FormatEndDt, ProjectName, PriorityIndex, HoursRequired ,StartDate,EndDate,FormatStDt,FormatEndDt,SourceType,Status FROM '" + db.TABLE_ACTIVITYMASTER_TEAM + "' WHERE PriorityIndex='1' AND UserMasterId ='" + UserMasterId + "'";
        Cursor cur = sql.rawQuery(query, null);
        if (cur.getCount() > 0) {
            return cur.getCount();
        }
    ...
    private int getunapprovedcnt() {
        SQLiteDatabase sql = db.getWritableDatabase();
        String query = "SELECT ActivityId,IssuedToName,ActivityName,Assigned_By,ActivityCode,ConsigneeName, SourceId,TotalHoursBooked, FormatStDt, FormatEndDt, ProjectName, PriorityIndex, HoursRequired ,StartDate,EndDate,FormatStDt,FormatEndDt,SourceType,Status FROM '" + db.TABLE_ACTIVITYMASTER_TEAM + "' WHERE UserMasterId ='" + UserMasterId + "' AND IsApproval ='true'";
        Cursor cur = sql.rawQuery(query, null);
        if (cur.getCount() > 0) {
            return cur.getCount();
        }
