...
import static com.vritti.vwblib.vworkbench.ClaimDetailsFragment.adapter;
...
public class ClaimSummaryFragment extends Fragment {
    ...
    public void addView_new(int i) {
        ...
        tv_distance.setText(ClaimDetailActivity.lsCalimDetails.get(i).getDistance());
    ...
    private void initialize(View rootView) {
        ...
        for (int i = 0; i < ClaimDetailActivity.lsCalimDetails.size(); i++) {
            addView_new(i);
        }
    ...
    public int gettotTravel() {
        int totTravel = 0;
        for (int i = 0; i < ClaimDetailActivity.lsCalimDetails.size(); i++) {
            totTravel = (int) (totTravel + Double.parseDouble(ClaimDetailActivity.lsCalimDetails.get(i).getTv_travelling()));
        }
        return totTravel;
    }
    ...
    public int gettotLocal() {
        int totTravel = 0;
        for (int i = 0; i < ClaimDetailActivity.lsCalimDetails.size(); i++) {
            totTravel = totTravel + Integer.parseInt(ClaimDetailActivity.lsCalimDetails.get(i).getTv_Local());
        }
        return totTravel;
    }
    ...
    public int gettotPhone() {
        int totTravel = 0;
        for (int i = 0; i < ClaimDetailActivity.lsCalimDetails.size(); i++) {
            totTravel = totTravel + Integer.parseInt(ClaimDetailActivity.lsCalimDetails.get(i).getTv_Ph());
        }
        return totTravel;
    }
    ...
    public int gettotFood() {
        int totTravel = 0;
        for (int i = 0; i < ClaimDetailActivity.lsCalimDetails.size(); i++) {
            totTravel = totTravel + Integer.parseInt(ClaimDetailActivity.lsCalimDetails.get(i).getTv_food());
        }
        return totTravel;
    }
    ...
    public int gettotRM() {
        int totTravel = 0;
        for (int i = 0; i < ClaimDetailActivity.lsCalimDetails.size(); i++) {
            totTravel = totTravel + Integer.parseInt(ClaimDetailActivity.lsCalimDetails.get(i).getTv_Maintenanace());
        }
        return totTravel;
    }
    ...
    public int getLogging() {
        int totlodg = 0;
        for (int i = 0; i < ClaimDetailActivity.lsCalimDetails.size(); i++) {
            totlodg = totlodg + Integer.parseInt(ClaimDetailActivity.lsCalimDetails.get(i).getTv_lodging());
        }
        return totlodg;
    }
    ...
    private JSONObject getJobj() {
        ...
        for (int i = 0; i < ClaimDetailActivity.lsCalimDetails.size(); i++) {
            ...
            try {
                ...
                ClaimDtlmain.put("Date", ClaimDetailActivity.lsCalimDetails.get(i).getClaimDate());
                ClaimDtlmain.put("FrmPlace", ClaimDetailActivity.lsCalimDetails.get(i).getFromPlace());
                ClaimDtlmain.put("ToPlace", ClaimDetailActivity.lsCalimDetails.get(i).getToPlace());
                String que = "SELECT Cd FROM " + db.TABLE_MODE_OF_JOURNY + " WHERE Desc_r='" + ClaimDetailActivity.lsCalimDetails.get(i).getTv_mode() + "'";
                Cursor cur = sql.rawQuery(que, null);
                if (cur.getCount() > 0) {
                    cur.moveToFirst();
                    ClaimDtlmain.put("JMode", cur.getString(cur.getColumnIndex("Cd")));
                }
                ClaimDtlmain.put("ModeName", ClaimDetailActivity.lsCalimDetails.get(i).getTv_mode());
                ClaimDtlmain.put("Travel", Double.parseDouble(ClaimDetailActivity.lsCalimDetails.get(i).getTv_travelling()));
                ClaimDtlmain.put("Local", Integer.parseInt(ClaimDetailActivity.lsCalimDetails.get(i).getTv_Local()));
                ClaimDtlmain.put("Lodging", Integer.parseInt(ClaimDetailActivity.lsCalimDetails.get(i).getTv_lodging()));
                ClaimDtlmain.put("Phone", Integer.parseInt(ClaimDetailActivity.lsCalimDetails.get(i).getTv_Ph()));
                ClaimDtlmain.put("Food", Integer.parseInt(ClaimDetailActivity.lsCalimDetails.get(i).getTv_food()));
                ClaimDtlmain.put("Distance", ClaimDetailActivity.lsCalimDetails.get(i).getDistance());
                ClaimDtlmain.put("RM", Integer.parseInt(ClaimDetailActivity.lsCalimDetails.get(i).getTv_Maintenanace()));
                ...
                ClaimDtlmain.put("lblDtlId", "");
                 detailTotal = Integer.parseInt(ClaimDetailActivity.lsCalimDetails.get(i).getTv_travelling()) +
                        Integer.parseInt(ClaimDetailActivity.lsCalimDetails.get(i).getTv_Local()) +
                        Integer.parseInt(ClaimDetailActivity.lsCalimDetails.get(i).getTv_Ph()) +
                        Integer.parseInt(ClaimDetailActivity.lsCalimDetails.get(i).getTv_food()) +
                        Integer.parseInt(ClaimDetailActivity.lsCalimDetails.get(i).getTv_lodging()+
                                Integer.parseInt(ClaimDetailActivity.lsCalimDetails.get(i).getTv_Maintenanace()));

                int tot = Integer.parseInt(ClaimDetailActivity.lsCalimDetails.get(i).getAmount());
    ...
    private void setValues() {
        String task = ClaimHeaderFragment.sp_task.getSelectedItem().toString();
        if (!task.equalsIgnoreCase("")) {
            textview_task.setText(task);
            String que = "SELECT ActivityId FROM " + db.TABLE_ACTIVITYMASTER + " WHERE ActivityName='" + ClaimHeaderFragment.sp_task.getSelectedItem().toString() + "'";
            Cursor cur = sql.rawQuery(que, null);
            if (cur.getCount() > 0) {
                cur.moveToFirst();
        ...
        String approver = ClaimHeaderFragment.sp_approver.getSelectedItem().toString();
        if (!approver.equalsIgnoreCase("")) {
            textview_approver.setText(approver);
            String que = "SELECT UserMasterId FROM " + db.TABLE_CLAIM_APPROVER + " WHERE UserName='" + ClaimHeaderFragment.sp_approver.getSelectedItem().toString() + "'";
        ...
        String cost = ClaimHeaderFragment.Sp_costCenter.getSelectedItem().toString();
        if (!cost.equalsIgnoreCase("")) {
            textview_costcenter.setText(cost);
            String que = "SELECT CostCtrMasterId FROM " + db.TABLE_COST_CENTER + " WHERE CostCtrDesc='" + ClaimHeaderFragment.Sp_costCenter.getSelectedItem().toString() + "'";
        ...
        String purpose = ClaimHeaderFragment.ed_travel_purpose.getText().toString();
