...
public class Util_Wifi_print {
    ...
    public static String connectionInfo(Activity mActivity) {
        ...
        for (NetworkInfo ni : netInfo) {
            if (ni.getTypeName().equalsIgnoreCase(Constants_wifi.CONTROLLER_WIFI)) {
                if (ni.isConnected()) {
                    result = Constants_wifi.CONTROLLER_WIFI;
                    break;
                }
            } else if (ni.getTypeName().equalsIgnoreCase(Constants_wifi.CONTROLLER_MOBILE)) {
                if (ni.isConnected()) {
                    result = Constants_wifi.CONTROLLER_MOBILE;
                    break;
                }
            }
        }
    ...
    public static void saveWifiConfiguration(Activity mActivity, WifiConfiguration mWifiConfiguration) {
        ...
        SharedPreferences mSharedPrefs = mActivity.getSharedPreferences(Constants_wifi.DEMO_PREFERENCES, Context.MODE_PRIVATE);
        mSharedPrefs.edit().putString(Constants_wifi.CONTROLLER_WIFI_CONFIGURATION, sJson).commit();
    }
    ...
    public static void savePrinterConfiguration(Activity mActivity, WifiConfiguration mPrinterConfiguration) {
        ...
        SharedPreferences mSharedPrefs = mActivity.getSharedPreferences(Constants_wifi.DEMO_PREFERENCES, Context.MODE_PRIVATE);
        mSharedPrefs.edit().putString(Constants_wifi.CONTROLLER_PRINTER_CONFIGURATION, sJson).commit();
    }
    ...
    public static WifiConfiguration getWifiConfiguration(Activity mActivity, String configurationType) {
        ...
        String mWifiJson = "";
        if (configurationType.equalsIgnoreCase(Constants_wifi.CONTROLLER_WIFI)) {
            mWifiJson = mSharedPrefs.getString(Constants_wifi.CONTROLLER_WIFI_CONFIGURATION, "");
        } else if (configurationType.equalsIgnoreCase(Constants_wifi.CONTROLLER_PRINTER)) {
            mWifiJson = mSharedPrefs.getString(Constants_wifi.CONTROLLER_PRINTER_CONFIGURATION, "");
        }
    ...
    public static void storeCurrentWiFiConfiguration(Activity mActivity) {
        try {
            @SuppressLint("WifiManagerLeak") WifiManager wifiManager = (WifiManager) mActivity.getSystemService(Context.WIFI_SERVICE);
